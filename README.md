...
| `CODER_INNER_CONTAINER_STOP_TIMEOUT` | The graceful stop timeout for the inner container before envbox forces shutdown. Use a Go duration such as `75s`.                                                                                                                                                                                                                                                                                                                                                                                                              | false    |
| `CODER_RECOVER_DOCKER_DATA_ROOT`     | If `true`, envbox restarts dockerd with an ephemeral vfs data-root when the mounted outer Docker data-root returns input/output errors. This preserves mounted workspace data such as `/home/coder`, but outer Docker images and containers are rebuilt for the pod until the persistent data-root is repaired or reset.                                                                                                                                                                                                       | false    |
