...
| Installing Sysbox node daemons is unacceptable, but modifying containerd configuration on an AL2023 MNG and accepting broader namespaced Pod permissions are acceptable | Native Kubernetes user namespaces with an additional stock-runc handler configured with `cgroup_writable = true` |
